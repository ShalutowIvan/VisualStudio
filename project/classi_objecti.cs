...
//             //  robots.Add(new robot("Tatiana", 100, new byte[] { 0, 1, 10 }));//обратились к названию списка robots и указываем добавление элементов Add, каждый элемент списка это должен быть объект на основе класса robot, для этого пишем new robot - robot это название класса, далее в скобках пишем параметры для объекта
...
//             kot_murzik Kotiara = new kot_murzik("Murzik", 200, new byte[] { 0, 0, 77 });//создали объект на основе второго наследника
//             Kotiara.printZnach(); //вывели значения в консоль

//             // robot kukish = new robot(150);//пока не убрали этот код чтобы проверить потом что не должен работать. Вот теперь нельзя так создать
//             // kukish.printZnach();
//             kot_murzik kukish = new kot_murzik();
//             System.Console.WriteLine();
//             kot_naslednik puma = new kot_naslednik("luna poluna", 150, new byte[] { 0, 1, 11 }, 120);
//             puma.printZnach();
//             puma.Lazer();//вызвали дополнительный метод


//             //теперь под определенный тип робота можно использовать нужный класс наследник, а не основной глобальный класс робот
//             //глобальный класс он общий, без определенного типа робота. Теперь нужно будет создавать робота на основе определенного типа, то есть на основе нужного класса наследника, а в родителе там будут просто основной функционал который будет передаваться наследникам, а в наследника будет дополнительный функционал
//             //абстрактные методы!!!!!!!!!!!!!
//             //теперь функцию вывода в консоль printZnach сделаем абстрактной, так как она используется везде во всех классах и его реализация везде должна быть разной, так как у разных роботов разные параметры и разное кол-во параметров. 
//             //если его сделать абстрактным, то он не будет реализован в классе родителе, но он будет реализован в других классах наследниках
//             //в классе робот перепишем эту функцию
//             //и в классах наследниках тоже перепишем эту функцию, так как она там вызывается
//             //абстрактный метод нужно обязательно реализовать во всех наследниках, иначе будет ошибка

//             //теперь у нас получился метод, он реализовался во всех дочерних классах
//             //в дочерних классах мы обращаемся к полям переменным из родительского класса через аксессоры, так как у них доступ закрыт указан модификатор доступ private
//             //с помощью аксессоров мы контролируем доступ к полям на получение данных и установку данных

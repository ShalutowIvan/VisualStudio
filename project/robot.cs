...
{
    abstract class robot//сделали класс абстрактным, теперь на основе робота нельзя создать объект
    {
        ...
        // public virtual void printZnach()//функция //указали что этот метод виртуальный написали virtual, это означает что его можно переписать в классе наследнике
        // {
        ...
        //     }
        // }
        public abstract void printZnach();//теперь функция абстрактная
        ...
        public byte[] Coordinates//аксессор для массива записывается так
        {
            get
            {
                return coordinates;//и тут просто название, без квадратных скобок
            }

            private set { }//к установке доступ закрыт


        }
        //абстрактные классы!!!!!!!!!!!
        //абастрактные классы это те классы на основе которых нельзя создать объект. в них описываются определенные методы, их можно реализовать внутри этого класса. 
        //также можно прописать абстрактный метод, который можно реализовать в классе наследнике
        //такие классы создаются для описания общего объекта или общего действия
        //потом создаются классы наследники, и все наследники получают общий функционал из абстрактного класса родителя и на основе наследников можно создавать объекты, а в родителе будут только характиристики, объекты на его основе не будет возможности создать объект.
        //в нашем случае класс robot будет абстрактным классом родителем, на его основе будут работать классы наследники, и на основе наследников будут создаваться объекты
